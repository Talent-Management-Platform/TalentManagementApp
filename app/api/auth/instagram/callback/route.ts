  ...
  try {
    ...
    const { data: account, error: dbError } = await supabaseAdmin
      ...
      .insert({
        ...
        platform: "instagram",
        is_connected: true,
        handle: "Connected Instagram Account",
      })
      .select()
      .single()

    if (dbError) {
      console.error("[v0] Error creating Instagram account:", dbError)
      return NextResponse.redirect("/dashboard/talents?error=db_error")
    }

    return NextResponse.redirect("/dashboard/talents?success=instagram_connected")
  } catch (error) {
    console.error("[v0] Error in Instagram callback:", error)
